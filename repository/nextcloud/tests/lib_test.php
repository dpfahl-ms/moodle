...
XML;
    ...
    protected function create_webdav_mock($openreturn = true, $searchreturn = null, $searchpath = '/remote.php/dav/',
                                        $searchuser = 'testuser', $searchquery = 'test', $expectsearch = true): MockObject {
        $mock = $this->createMock(\webdav_client::class);
     ...
     * @return \PHPUnit\Framework\MockObject\MockObject
     */
    protected function create_oauth_mock($userinfo = ['username' => 'testuser']): MockObject {
        $mock = $this->createMock(\core\oauth2\client::class);
     ...
     * @covers ::search
     */
    public function test_search_errors($searchtext, $userinfo, $expecteddebugging, $openreturn = true, $expectsearch = true): void {
        $this->resetAfterTest(true);
        $this->setUser();
        $expected = $this->get_initialised_return_array();
        ...
        if ($userinfo === null) {
            ...
            $this->create_webdav_mock($openreturn, null, '/remote.php/dav/',
                $userinfo['username'] ?? '', $searchtext, $expectsearch);
