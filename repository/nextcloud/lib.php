...
class repository_nextcloud extends repository {
    ...
    public function search($searchtext, $page = 0) {
        ...
        $this->initiate_webdavclient();
        if (empty($this->client) || !$this->dav->open()) {
            return $ret;
        }
        ...
        // Manually setting the dav entpoint different to the one from the nextcloud issuer.
        // The origianle davendpoint /remote.php/webdav does not support SEARCH.
        ...
        if (is_array($results)) {
            ...
            // Parameter endpointurl for proper href parsing, since they have a different
            // base urls in the search request response, which includes the username.
            $ret['list'] = $this->get_listing_convert_response('/', $results, $searchdavpath . 'files/' . $username);
        }
     ...
     */
    private function get_listing_convert_response($dirpath, $ls, $endpointurl = '') {
        global $OUTPUT;
        $folders = array();
        $files = array();
